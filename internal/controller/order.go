...
func CreateAOrder(c *gin.Context)  {
	...
	// 创建订单使用的额外服务(多个)
	for _, orderExtraService := range apiOrder.ExtraServiceList {
		orderExtraService.ID=util.UniqueId()
		orderExtraService.OrderId=apiOrder.Order.ID
	}
	err=dao.CreateOrderExtraServiceList(apiOrder.ExtraServiceList)
	if err !=nil{
		util.RespJSON(c,gin.H{
			"err":err,
		})
	...
	// 创建订单使用的附加服务
	for _, orderCouponList := range apiOrder.OrderCouponList {
		orderCouponList.ID=util.UniqueId()
		orderCouponList.OrderId=apiOrder.Order.ID
	}
	err=dao.CreateOrderCouponList(apiOrder.OrderCouponList)
	if err !=nil{
		util.RespJSON(c,gin.H{
			"err":err,
		})
